#!/bin/bash
#SBATCH --job-name=kv_quant_rerun_fidelity_mistral
#SBATCH --partition=ai-h100l-pu
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --time=30:00
#SBATCH --output=logs/rerun_fidelity_mistral_%j.log
#SBATCH --error=logs/rerun_fidelity_mistral_%j.err

# 忠実度 (logit/KV fidelity) のみを再測定する。【Mistral-7B 側 / 分割 2本のうち 2本目】
#
...
# モデルごとに分割して 2 ジョブ並列で実行する (本スクリプト: Mistral 側。
# もう一方: run_rerun_fidelity_ai_h100l_pu_llama.sh)。
#
# 【計算資源についての特例】
...
#
# 対象 (計5本):
#   - fidelity: 全5手法 (fp16 含む) x Mistral-7B-Instruct-v0.3, seq_len 1024
...
echo " Split job: Mistral-7B-Instruct-v0.3 only (2 of 2)"
