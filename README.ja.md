...
- **Logit 忠実度**：最終 logits のコサイン類似度・Top-1一致率・Top-5**重なり率**（seq 1024、非反復の自然文を使用） → `eval_pt/eval_fidelity.py`
  - 注意：反復的な合成文を入力にすると、次トークン予測のマージンがほぼ全位置で大きくなり Top-1 が飽和して（例えば全条件で 1023/1024≈99.90% に張り付き）識別力を失います。下表の Top-1 列はこの飽和条件下の旧測定値のため参考値です。
...
> **Top-1 列についての注意**：本表は旧スクリプト（反復的な合成文を使用）での測定値であり、Top-1 は全条件で 99.5〜99.90% に飽和しています（1023/1024≈99.90% は「1024 位置中 1 位置のみ argmax が反転」を意味する格子点。反転は BOS 近傍の低マージン位置に集中）。評価スクリプトは非反復の自然文に修正済みで、再測定により本列は更新予定です。飽和の影響を受けない logit cos・Top-5・KV 誤差の各列は有効な測定値です。
...
> 観察：Mistral-7B では全手法が高い logit cos（0.977〜0.999）を維持する一方、turbo/hyper は Top-5 重なりが 59.9〜66.3% まで落ちます（rotor/ultra は 94%台）。Llama-3.1-8B では 3bit・per-tokenスケールの手法（turbo/hyper）の劣化がさらに大きく（cos 0.685〜0.707）、KV 相対L2誤差も 0.62〜0.66 に達します。rotor/ultra は細粒度スケール（3Dブロック / 4bitグリッド）ゆえに高い — ベースモデル依存の耐性差が如実に出ています。
...
PPL・logit/KV忠実度・LongBench QA-F1 のいずれの指標でも、Llama-3.1-8B は Mistral-7B より 3bit per-token手法（turbo/hyper）での劣化が大きい傾向が一貫しています。端タスクでも turbo は Llama で fp16比ほぼ半減（0.1580 vs 0.2997）するのに対し、Mistral ではほぼ維持（0.3135 vs 0.3256）です。アーキテクチャ依存の耐性差は実在し、本実装の範囲では万能な手法はありません。なお品質・footprint・遅延の総合バランスでは ultra_quant（4bit、272MB、遅延約1.3倍）が最も堅い結果でした。
