...
> **例外**：忠実度評価（job 373801）のみ、実行時点で ai-l40s が利用不可だったため **ai-h100l-pu（NVIDIA H100）** パーティションを使用しました（`hpc_scripts/run_rerun_fidelity_ai_h100l_pu_llama.sh` / `run_rerun_fidelity_ai_h100l_pu_mistral.sh` の2分割ジョブ。pu パーティションは最大実行時間30分のためモデルごとに分割）。量子化器の数値結果はデバイスに依存しないため結果への影響はありません。速度値はすべて ai-l40s での測定値であり、このジョブでは測定していません。
...
**passthrough**（量子化を行わない疑似量子化器）が fp16 と**トークン完全一致**することのみを `overall_pass` の判定対象とします → キャッシュ配管（蓄積・連結・全履歴返却）の正当性を量子化誤差から分離して検証します。
...
- **Logit 忠実度**：最終 logits のコサイン類似度・Top-1一致率・Top-5**重なり率**（seq 1024、非反復の自然文を使用） → `eval_pt/eval_fidelity.py`
  - 注意：反復的な合成文を入力にすると、次トークン予測のマージンがほぼ全位置で大きくなり Top-1 が飽和して（例えば全条件で 1023/1024≈99.90% に張り付き）識別力を失います。この飽和を避けるため、本評価では非反復の自然文を使用しています。
...
> 🗂️ **結果の出処**：以下の表は測定ジョブ（373124・373434・373457・373515・373801）の結果を機械集計したものです。`results/ai-l40s/*.json` を機械集計した全表は [`results/ai-l40s/valid_results_summary.md`](results/ai-l40s/valid_results_summary.md)、マージ済みデータは `python results/summarize_results.py` で生成した [`summary_results.csv`](summary_results.csv) です。
...
参考：シミュレーションの**実際の保存占有**（int8＋fp32スケール、パッキング無し）は fp16=1,024 MB、turbo/hyper/ultra=528 MB、rotor=1,208 MB。ビット幅に関わらず全手法が同じ値になるのは**保存形式の自明な帰結**であり、この値から設計ビットの優劣を議論することはできません（↑の解析的表を使うのが正しい扱い）。

![圧縮率の比較](plots/compression_comparison.png)

### 2. 品質指標 — PPL、logit / KV 忠実度

2026-08 に測定した値です。量子化器は seed 固定で、HyperQuant には D4格子射影を適用しています。
...
> **測定条件**：2026-08-27 に非反復の自然文で測定（job 373801、ai-h100l-pu/H100 ※数値結果はGPU機種に非依存）。反復的な合成文では Top-1 が飽和して識別力を失うため（品質指標の注を参照）、自然文を使用しています。自然文では不一致位置が系列全体に分散し、Top-1 も手法間を弁別する指標として機能しています。
...
> 観察：3bit・per-tokenスケールの手法（turbo/hyper）は Llama-3.1-8B で Top-1 が 91%台・logit cos が 0.82〜0.85 まで低下するのに対し、Mistral-7B では Top-1 92〜93%・cos ≥0.98 に留まります。rotor/ultra は細粒度スケール（3Dブロック / 4bitグリッド）ゆえ両モデルで高忠実度（Top-1 96.6%以上、cos ≥0.987）。Top-1 にもベースモデル依存の耐性差が現れていると考えられます。
...
各深度5試行（計25試行）で測定しています。モデル依存性の向きは PPL・忠実度と逆です：検索タスクでは Llama-3.1-8B が全手法パーフェクトなのに対し、Mistral-7B では 3bit手法が劣化（turbo 0.64 / rotor 0.76 / hyper 0.20）。ultra_quant は両モデルで検索耐性があります。
...
全量子化手法が fp16 を下回ります（=量子化器の追加コスト）。rotor は 3Dブロック毎スケール処理が重く最大の遅延（2.35〜2.45倍）です。
...
シミュレーションでは全手法を int8 で保存するため、実測の保存バイト数はビット幅に関わらず一律 528 MB に収束します — これは発見ではなく保存形式の同語反復と考えられます。そのため解析的 footprint（設計ビットのデータ量＋各実装のスケール保存形式に基づくメタデータ）で比較するのがより誠実な扱いだと考えられ、実環境の数値にはさらにビットパッキングが必要になると見られます。
...
- **Top-1 は入力設計に依存する指標です**：反復的な合成文ではマージンがほぼ全位置で大きく argmax が反転せず、全条件で飽和して識別力を失います（1023/1024≈99.90% は「ちょうど1位置の反転」という格子点で、反転は BOS 近傍の低マージン位置に集中）。非反復の自然文では Top-1 は手法を正しく弁別し（Llama の turbo/hyper は 91%台、rotor/ultra は 97%前後）、PPL・QA-F1 の劣化と整合します。fp16 同士の比較では Top-1=100.00% であり、測定の正常性も確認済みです。→ **Top-1 は入力設計次第で劣化を見落としうる指標であり、分布指標（cos・Top-5）との併用が必要** だと考えられます。
...
- なお **試行回数の確保は評価の分解能を左右しうる** と考えられ、本評価も25試行規模のため、厳密な優劣ではなく傾向として解釈すべき結果です。
...
- **統計的分解能の不足**：NIAH は各深度5試行（計25試行）、LongBench は先頭10サンプルのみの小規模評価であり、厳密な優劣ではなく傾向の把握に留まります。
...
- **評価規模の拡大**：LongBench のサンプル数拡大や NIAH のさらなる試行回数の増加により、統計的に信頼できる結論を得る。
...
全指標の計測結果が出揃い、量子化手法の優劣はベースモデルやスケーリングの粒度に依存する傾向が示唆されました。品質・footprint・遅延の総合バランスでは、**ultra_quant（4bit、272MB、遅延約1.3倍）が最も頑健な選択肢である** と考えられます。
